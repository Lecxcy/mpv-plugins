...
- **模板**（详细设计见 [SPEC.md §10](SPEC.md#10-模板templates)）：9 个编号
  槽位（裸 `1`..`9` 应用 / `Ctrl+1..9` 保存 / `Alt+1..9`
  删除），每个槽位是一份独立的 `(segments, pending)` 完整快照，不是对当前
  segments 的引用或一份启用掩码——最简单的用法是同一批区间、不同槽位只是
  `enabled` 不同，也支持每个槽位存完全不同的区间。保存/删除只改内存，仍然
  要 `Ctrl+S` 才连同 segments/pending 一起落盘。应用键位换过三轮：先是
  `meta+1..9` 撞上了 mpv 自己 macOS 原生窗口菜单的"实际大小"/"双倍大小"
  （`Cmd+1`/`Cmd+2`，比 `input.conf` 更底层，且 mpv 是命令行启动、不是独立
  `.app`，没法用系统"App 快捷键"让路）；改成 `Shift+1..9` 又踩到 mpv 对可
  打印字符不支持 `Shift+` 前缀写法这条规则（要写 Shift 按下后实际产生的字符
  本身，跟这个仓库里 `unset-a`/`unset-b` 一直用字面 `{`/`}` 而不是
  `Shift+[`/`Shift+]` 是同一条规则），于是落成 US 键盘布局下的 `!@#$%^&*(`
  （数字 3 对应的 `#` 因为会被 mpv 当整行注释，还要改用按键别名 `SHARP`）；
  最后在 2026-07-26 的全仓库键位重排里改成裸数字键 `1`..`9`——`!@#$%^&*(`
  绑死 US 布局、换个键盘就整组失效，而"应用"是这三个动作里最高频的，本来就
  该配最便宜的键。代价是覆盖掉 mpv 内置的画质微调（1/2 对比度、3/4 亮度、
  5/6 gamma、7/8 饱和度、9 音量 -2）。完整排查过程见
...
- **存档/读档**：`save-loops`/`load-loops`，用内容采样哈希（文件大小 +
  ...
  做法，实现已提取到 `shared/cpp/confirm_lockout.h` 与 split-zoom-box 共用）；
  提示期间会强制暂停播放，并临时启用一个独占 input section 挡掉
  除 `confirm-yes`/`confirm-no` 实际绑定的物理键之外的所有按键（反查
  `input-bindings` 得到具体是哪个键，不硬编码，找不到就安全退化成不锁
  键），逼用户先回答完再继续操作；回答完成后把 pause 还原成确认前的状态。
  反查时是把 `input-bindings` 报的 cmd 按 `;` 拆成若干条命令再逐条比对的：
  `y`/`n` 现在由本插件和 split-zoom-box 共用，`input.conf` 里是
  `y script-binding a/confirm-yes ; script-binding b/confirm-yes` 这样一整
  行，早先的整串全等比较会比不上、反查落空，进而触发"找不到就不锁"的退化
  分支，**锁键静默失效**。
...
- **与 [uosc](../uosc/README.md) 进度条集成**：`refresh_loop`（唯一的
  ...
  列表发布到 `user-data/enhanced-ab-loop/segments` 这个 mpv `user-data`
  属性（专门给"脚本/插件之间共享任意数据"设计的机制，可读可写可
  observe）。`publish_segments_property` 把它构造成原生
  `MPV_FORMAT_NODE`（array of `{a, b, enabled}`）手动写入，**不是**
  `mpv_set_property_string` 写一份 JSON 字符串——最初就是这么写的，联调时
  发现 uosc 那边用 `mp.observe_property(..., 'string', ...)` 拿到字符串后
  传给 `mp.utils.parse_json` 会静默返回原字符串本身、而不是解析出的
  table（同一个 `parse_json` 处理硬编码的字符串字面量完全正常，只有从
  `observe_property` 回调参数拿到的字符串会触发这个问题，没有深挖 mpv
  Lua 绑定内部的根因），导致 uosc 那边 `state.ab_loop_segments` 永远是空
  表、进度条上什么都画不出来，且没有任何报错——排查过程见下方"验证"）。
  改成原生 node + `observe_property(..., 'native', ...)` 后 uosc 直接拿到
  Lua table，完全绕开这个坑。`plugins/uosc/lua/main.lua` observe 这个
  属性，`elements/Timeline.lua` 在进度条上把每一段区间的范围和 `enabled`
  状态画出来（enabled 用绿色，disabled 用暗淡描边），并通过播放位置是否
  落在某段自己的 `[a,b)` 内识别出"当前激活"的那一段、加亮它的边框——特意
  不依赖 `ab-loop-a`/`ab-loop-b`/`loop_enabled`，这样主循环开关关掉之后
  区间依然会显示，只是不再被 mpv 实际强制循环。这条集成只对本仓库的
  `plugins/uosc` 派生版生效，不影响 `external/plugins/uosc` 这份上游参考。
...
- uosc 集成：用同一套 IPC 驱动方式，插入两个区间（`[2,6]` 与 `[10,14]`）
  ...
  数据；对第二段执行 `toggle-segment` 后属性实时更新为 `enabled:false`；
  重新打开循环后光标落回第一段，`ab-loop-a`/`ab-loop-b` 精确等于
  `2.0`/`6.0`，与该段自己的 `a`/`b` 一致。**教训**：最初用
  `mpv_set_property_string` + JSON 字符串这条链路时，`get_property`/IPC
  层面看起来完全正常、mpv 日志全程没有任何 Lua 报错，但 uosc 进度条上其实
  什么都没画出来——问题出在 `mp.utils.parse_json` 处理"从
  `observe_property` 回调参数拿到的字符串"时会静默失败并原样返回输入，不
  抛错也不返回 `nil`。这类"没报错但结果不对"的坑，光看 IPC 返回值和日志
  是测不出来的，必须用 `screenshot-to-file ... window` 截一张真实窗口
  的图、逐像素采样确认颜色，外加对照组（先用硬编码字符串字面量测通过、再
  用真实回调参数测，逐步缩小范围）才定位到具体是哪一步在说谎。改成原生
  `MPV_FORMAT_NODE` + `observe_property(..., 'native', ...)` 后用同样的
  截图验证过颜色确实出现在两段区间各自的位置上。
...
- 2026-07-25：`refresh_loop` 新增把 segments 发布到
  ...
  enabled/激活状态，此前 SPEC.md §7 记录的这条已知限制解除；同一天发现
  最初用 JSON 字符串发布会在 uosc 侧被 `mp.utils.parse_json` 静默解析
  失败（见上方"验证"的教训），改成原生 `MPV_FORMAT_NODE` 发布解决。
