...
for name in "${lua_plugins[@]}"; do
    ...
    if [[ -f "${repo_root}/plugins/${name}/${name}.conf" ]]; then
        ...
        cp "${repo_root}/plugins/${name}/${name}.conf" "${dist_dir}/script-opts/${name}.conf"
    fi

    if [[ -d "${repo_root}/plugins/${name}/fonts" ]]; then
        mkdir -p "${dist_dir}/fonts"
        cp -R "${repo_root}/plugins/${name}/fonts/." "${dist_dir}/fonts/"
    fi
done
...
echo ""
echo "注意：除上方 lua_plugins 列表登记的插件外，其余尚未 C++ 重写的插件"
echo "（纯 Lua）不在收集范围内，仍需按各自 README 中的方式单独加载"
echo "（例如 mpv --script=plugins/<名字>/lua/<名字>.lua）。"
echo ""
echo "测试方式："
