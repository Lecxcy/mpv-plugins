...
# 刻意不设 CFBundleTypeIconFile：让 macOS 自己按 UTI 合成那种带格式角标的
# 通用文档图标（就是 .mp4 现在的样子）。我们只是个转发壳，没理由把视频文件
# 都画成自己的图标。
...
if skeleton:
    ...
    for d in skel.get("UTImportedTypeDeclarations", []):
        if d.get("UTTypeIdentifier") not in wanted:
            continue
        d = dict(d)
        # 必须剥掉。骨架里每条都写着 UTTypeIconFile=document.icns，而那个文件
        # 在我们的 bundle 里不存在——IconServices 按声明去找一个不存在的文件，
        # 会返回一张空图，访达里这些格式的图标整个消失，连点击热区都没有。
        #
        # 踩过：avi/mkv/flv/m4v/rm 全变空白，mp4/mov/webm/wmv 正常，分界线正是
        # "这个 UTI 在不在导入列表里"。排查时先怀疑了硬盘、图标缓存、默认打开
        # 方式，最后才发现是自己搬声明时带进来一个悬空引用。
        #
        # 删掉键 ≠ 没图标：系统会回退到自己合成的通用文档图标。
...
print(f"  声明了 {len(utis)} 个 UTI，搬运了 {len(imported)} 条 UTI 定义（图标交给系统合成）")
PY
